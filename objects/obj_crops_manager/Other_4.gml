...
//if (room = rm_farm) {
	...
	if ds_crops_data[# 0, 0] != -1 {
		...
		var _slot = 0; repeat(_inst_num) {
			respawn_crop(
				...
				ds_crops_data[# 5, _slot],
				ds_crops_data[# 6, _slot]
				)
			_slot +=1
		}
	}
	...
	with(obj_fantasy_crop) {
		...
		var _cur_exp = get_cur_exp(days_old,days_not_watered,bonus)
		sprite_num = get_crop_spr_num(_exp, _state_5_exp, max_sprite_num)
